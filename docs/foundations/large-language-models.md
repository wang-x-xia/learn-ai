...
> 注意力变体（MHA/GQA/MLA）及 KV Cache 优化详见 [KV Cache 与推理优化](./kv-cache.md)；MoE 等架构层面的技术详见 [Transformer 架构](./transformer.md)；SSM/Mamba 相关内容见 [Mamba 与状态空间模型](./mamba-and-ssm.md)。本节聚焦 LLM 特有的技术维度。
...
长上下文的关键技术：
- **RoPE 频率外推**: 将 RoPE 基础频率调大，使模型支持超出训练长度的上下文
- **持续预训练**: 在长文本上继续训练
- **Flash Attention**: IO 感知的注意力算法，内存从 O(n²) 降到 O(n)，2-4x 加速且结果精确等价
